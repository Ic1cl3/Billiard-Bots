...
var flippedDefaultTempOut : Dictionary = {
	"direction" : 180.0
}
...
func openBot(path : String, botNum : int) -> void:
	...
	if botNum == 0:
		...
		botTwoPath = path
		tempOut.store_string(JSON.stringify(flippedDefaultTempOut))
	tempIn.close()
	tempOut.close()
...
## Retrieves the direction property (only bot output property) of the given bot. Aims toward center if there is an error retrieving the data.
func getDir(bot : int):
	if bot == 0 and FileAccess.file_exists(botOnePath + "/tempOut.json"): # Only proceed if file exists.
		...
		if FileAccess.get_open_error() == 0: # Only proceed if there is no error opening the file.
			...
			tempOut.close() # Close file if opening failed.
	elif bot == 1 and FileAccess.file_exists(botTwoPath + "/tempOut.json"): # Mirrored from top, but with bot two path.
	...
	if bot == 0:
		...
		return 180 # Flips to 180 for bots which start on the right side of the screen, keeping the game fair in a mutual bot failure.
